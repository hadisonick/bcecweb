import _objectSpread from "@babel/runtime/helpers/objectSpread2";
import React from "react";
import styled from "@emotion/styled";
import kebabCase from "lodash/kebabCase";
import { DisplayMode } from "../../../../types";
import { createInitialPerDeviceSettingValue } from "../../elementSettings/elementSettingsUtils"; // Icons

import { ReactComponent as IconSvg } from "@svgr/webpack!./round-star_border-24px.svg"; // Components

import IconSettings from "./IconSettings";
import Icon from "./Icon";
import { getSvg } from "../utils/iconUtils";
export default (function () {
  var args = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var PreviewBox = /*#__PURE__*/styled("div", {
    target: "e1dhgwzy0",
    label: "PreviewBox"
  })({
    textAlign: "center",
    height: 50,
    color: "var(--mdc-theme-text-primary-on-background)",
    svg: {
      height: 50,
      width: 50
    }
  });
  var elementType = kebabCase(args.elementType || "icon");
  var defaultToolbar = {
    title: "Icon",
    group: "pb-editor-element-group-basic",
    preview: function preview() {
      return /*#__PURE__*/React.createElement(PreviewBox, null, /*#__PURE__*/React.createElement(IconSvg, null));
    }
  };
  var defaultSettings = ["pb-editor-page-element-style-settings-icon", "pb-editor-page-element-style-settings-padding", "pb-editor-page-element-style-settings-margin", ["pb-editor-page-element-style-settings-horizontal-align", {
    alignments: ["left", "center", "right"]
  }], "pb-editor-page-element-settings-clone", "pb-editor-page-element-settings-delete"];
  return [{
    name: "pb-editor-page-element-".concat(elementType),
    type: "pb-editor-page-element",
    elementType: elementType,
    toolbar: typeof args.toolbar === "function" ? args.toolbar(defaultToolbar) : defaultToolbar,
    settings: typeof args.settings === "function" ? args.settings(defaultSettings) : defaultSettings,
    target: ["cell", "block"],
    create: function create(options) {
      var defaultValue = _objectSpread({
        type: this.elementType,
        elements: [],
        data: {
          icon: {
            id: ["far", "star"],
            svg: getSvg(["far", "star"], {
              width: 50
            }),
            width: 50
          },
          settings: {
            horizontalAlign: "center",
            margin: createInitialPerDeviceSettingValue({
              all: "0px"
            }, DisplayMode.DESKTOP),
            padding: createInitialPerDeviceSettingValue({
              all: "0px"
            }, DisplayMode.DESKTOP)
          }
        }
      }, options);

      return typeof args.create === "function" ? args.create(defaultValue) : defaultValue;
    },
    render: function render(props) {
      return /*#__PURE__*/React.createElement(Icon, props);
    }
  }, {
    name: "pb-editor-page-element-style-settings-icon",
    type: "pb-editor-page-element-style-settings",
    render: function render() {
      return /*#__PURE__*/React.createElement(IconSettings, null);
    }
  }];
});
//# sourceMappingURL=index.js.map