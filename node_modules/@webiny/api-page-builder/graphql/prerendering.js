...
var _default = [new _PagePlugin.PagePlugin({
  ...
  async afterUnpublish({
    ...
    const promises = [];
    promises.push(context.pageBuilder.pages.prerendering.flush({
      paths: [{
        path: page.path
      }]
    })); // After a page was unpublished, we need to rerender pages that contain pages list element.

    promises.push(context.pageBuilder.pages.prerendering.render({
      tags: [{
        tag: {
          key: "pb-pages-list"
        }
  ...
  async afterPublish({
    ...
    var _settings$pages, _settings$pages2;
    ...
    const settings = await context.pageBuilder.settings.default.getCurrent(); // If we just published a page that is set as current homepage, let's rerender the "/" path as well.

    if ((settings === null || settings === void 0 ? void 0 : (_settings$pages = settings.pages) === null || _settings$pages === void 0 ? void 0 : _settings$pages.home) === page.pid) {
      promises.push(context.pageBuilder.pages.prerendering.render({
        paths: [{
          path: "/"
        }]
    ...
    if ((settings === null || settings === void 0 ? void 0 : (_settings$pages2 = settings.pages) === null || _settings$pages2 === void 0 ? void 0 : _settings$pages2.notFound) === page.pid) {
      promises.push(context.pageBuilder.pages.prerendering.render({
        paths: [{
          path: page.path,
          configuration: {
...
}), new _SettingsPlugin.SettingsPlugin({
