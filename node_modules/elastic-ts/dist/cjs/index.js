...
function buildAggregation3([name, type, fieldOrConfig]) {
    if (is__default['default'].string(fieldOrConfig)) {
...
function buildAggregation4([name, type, fieldOrConfig, configOrBuilder]) {
    if (is__default['default'].string(fieldOrConfig)) {
        const field = fieldOrConfig;
        if (is__default['default'].function_(configOrBuilder)) {
...
function buildClause3([type, fieldOrConfig, valueOrConfigOrBuilder]) {
    if (is__default['default'].string(fieldOrConfig)) {
        const field = fieldOrConfig;
        if (is__default['default'].function_(valueOrConfigOrBuilder)) {
            const builder = valueOrConfigOrBuilder;
            return { [type]: { [field]: builder().build() } };
        }
        if (is__default['default'].plainObject(valueOrConfigOrBuilder)) {
...
function buildSort1([fieldOrConfigArray]) {
    ...
    return configArray.map((item) => {
        if (is__default['default'].string(item)) {
            ...
            if (is__default['default'].string(orderOrConfig)) {
...
function buildBody(data) {
    ...
    if (is__default['default'].nonEmptyObject(aggregations)) {
...
function prepareSubBuilderArgs(args, replacement) {
    args.forEach((arg, i) => {
        if (is__default['default'].function_(arg)) {
